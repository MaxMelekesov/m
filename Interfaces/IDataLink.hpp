...
struct RingSpan {
  ...
  std::optional<std::span<uint8_t>> copyTo(
...
*/
template <typename T>
concept CRingDataLink = requires(T dl, std::span<uint8_t> rx_buf,
...
*/
template <typename T>
concept CDataLink = requires(T dl, std::span<uint8_t> rx_buf,
                             std::span<const uint8_t> tx_buf) {
  { dl.startReceive(rx_buf) } -> std::same_as<bool>;
  {
    dl.getPacket()
