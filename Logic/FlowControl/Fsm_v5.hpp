    ...
    static consteval void validate() {
        ...
        for (std::size_t i = 1; i < arr.size(); ++i)
            if (arr[i-1].from == arr[i].from && arr[i-1].ev == arr[i].ev)
                throw std::logic_error("Duplicate (State, Event) transition");
        ...
        for (const auto& t : table_)
            if (!valid(t.from) || !valid(t.to) || !valid(t.ev))
                throw std::logic_error("Invalid enum value in transition table");
    }
...
public:
    using Fsm = fsm::machine<State, Event, Robot,
        fsm::transition{State::Idle,    Event::Move,      State::Moving,  &Robot::can_move,      &Robot::on_move},
        fsm::transition{State::Moving,  Event::Obstacle,  State::Blocked, &Robot::can_block,     &Robot::on_block},
        fsm::transition{State::Blocked, Event::Clear,     State::Idle,    &Robot::can_clear,     &Robot::on_clear}
    >;
    Fsm fsm{State::Idle};
