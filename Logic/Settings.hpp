...
template <typename T>
struct IsTagValue : std::false_type {};
...
class Settings {
 ...
 public:
  ...
  void setValue(ValueType&& value) {
    static_assert(std::is_base_of_v<TagBase, Tag>, "Tag must be derived from TagBase");
    static_assert(tagExists<Tag>(), "Tag not found in settings pairs");
    
    constexpr size_t index = getIndex<Tag>();
    using ActualValueType = std::tuple_element_t<index, decltype(storage.values)>;
  ...
  typename Tag::ValueType getValue() const {
    static_assert(tagExists<Tag>(), "Tag not found in settings pairs");
 ...
 private:
  template <typename Tag>
  static constexpr bool tagExists() {
