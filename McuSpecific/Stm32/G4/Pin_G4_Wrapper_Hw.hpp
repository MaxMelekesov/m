...
#ifndef PIN_G4_WRAPPER_HW_HPP
#define PIN_G4_WRAPPER_HW_HPP

#include <Hardware.hpp>
#include <Pin_G4.hpp>

template <typename TagT>
class PinWrapper final : public m::mcu::Peripheral<TagT>,
                         public m::ifc::mcu::IPin {
 public:
  using HwKey = typename m::mcu::Peripheral<TagT>::HwKey;

  explicit PinWrapper(HwKey key, GPIO_TypeDef* port, Pin::PinNum pin_num,
                      Pin::Mode mode,
                      Pin::Inversion inversion = Pin::Inversion::Not_Inverted,
                      Pin::InitState init_state = Pin::InitState::Reset)
      : m::mcu::Peripheral<TagT>(key),
        port_(port),
        pin_num_(pin_num),
        mode_(mode),
  ...
  ~PinWrapper() override = default;
  PinWrapper(const PinWrapper&) = delete;
  PinWrapper& operator=(const PinWrapper&) = delete;
  PinWrapper(PinWrapper&&) = default;
  PinWrapper& operator=(PinWrapper&&) = delete;
